...
function DropDown(props) {
  ...
  const handleGroupingChange = (ev)=>{
     localStorage.setItem("grouping",ev.target.value);
     props.onGroupingOptionChange(ev.target.value);
  }

  const handleSortingChange = (ev)=>{
    localStorage.setItem("ordering",ev.target.value);
    props.onSortingOptionChange(ev.target.value);
  }
  ...
  return (
    <div className="dropdown">
      <div>
        <button className="dropdown-head" onClick={() => setOpen(!open)}>
          <svg
            width="16"
            height="16"
            viewBox="0 0 16 16"
